...
- 1.1.0(未发布): 修复模式 2/4 打包旧内容、新增文件失败、`changes.txt` 未进包、`ignore_dir_prefix` 误替换、PHP 8.1+ 弃用告警; 移除 `alchemy/zippy` 依赖改用内置 `PharData`; 新增 `--help` / `--version` / `--dry-run` 与 `packme.ini` 配置项。
